...
%
% ADDEME: Precomputes filter transfer functions. All computations are 
% done on the Fourier domain. 
%
...
% Transfer functions:
G=zeros([M N Nfilters]);
for i=1:Nfilters
    par=param(i,:);
    tr=t+param(i,4); 
    tr=tr+2*pi*(tr<-pi)-2*pi*(tr>pi);

    G(:,:,i)=exp(-10*param(i,1)*(fr/(sqrt(M)*sqrt(N))/param(i,2)-1).^2-2*param(i,3)*pi*tr.^2);
end
