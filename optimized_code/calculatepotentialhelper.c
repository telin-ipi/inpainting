...
void calculatepotentialhelper(const int M, const int N, 
			const double *img, const int nlab, const double *lab,
            const int nnbrlab, const double *neighbourLabels, const double *ind1, 
            const double *ind2, const int gap, double *pot) 
{
    register int l,k,p,x,y,xx,yy,i,imask,j,jmask,ndxPot=0,ndx,ndxPatch,ndxMask,numPix=M*N,sizePatch=2*gap+1,sizeOverlap=(gap+1)*(2*gap+1);
    ...
    for (l=0; l<=nlab-1; l++) {
        ...
        /* compare with all the labels of the neighbouring node */
        for (k=0; k<=nnbrlab-1; k++) {
...
/* pot = calculatepotentialhelper(M, N, img, nlab, lab, nnbrlab, neighbourLabels, ind1, ind2, gap); */
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) 
{
  int M,N,nlab, nnbrlab, gap, i;
  double *img, *lab, *neighbourLabels, *pot, *ind1,*ind2;
  ...
  lab = mxGetPr(prhs[4]);
  nnbrlab = (int)mxGetScalar(prhs[5]);
  ...
  /* Setup the output */
  plhs[0] = mxCreateDoubleMatrix(1, nlab*nnbrlab, mxREAL);
  pot = mxGetPr(plhs[0]);
  for (i=0; i<=nlab*nnbrlab-1; ++i)
      pot[i] = 0.0;
  

  /* Do the actual work */
  calculatepotentialhelper(M, N, img, nlab, lab, nnbrlab, neighbourLabels, ind1, ind2, gap, pot);
}
